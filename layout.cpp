                                                 ...
                                                 cogui::container * cont)
{
    ...
    int cx = 1;
    int cc = 1;
    bool expandable = (m_expanded_cell != -1);
    ...
    if(expandable)
    {
        ...
        for(int i = m_expanded_cell; i<controls.size(); i++)
        {
        ...
        for(int i=0; i<controls.size(); i++)
        {
            ...
            if(i == m_expanded_cell)
            {
                cx = cont->getWidth() - accumulated_width - m_expanded_cell;
            }
...
{
    m_expanded_cell = c;
    info() << "Setting expand cell:" << m_expanded_cell;
    if(m_container)
    {
        debug() << "Relayouting";
